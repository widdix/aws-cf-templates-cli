...
const fetchProfiles = async (file, profiles) => {
...
module.exports.fetchProfiles = async () => {
  ...
  if (fs.existsSync(CONFIG_FILE_PATH)) {
    profiles = await fetchProfiles(CONFIG_FILE_PATH, profiles);
  }
  if (fs.existsSync(CREDENTIALS_FILE_PATH)) {
    profiles = await fetchProfiles(CREDENTIALS_FILE_PATH, profiles);
  }
